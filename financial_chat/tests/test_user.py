...
class UserTestCase(unittest.TestCase):
    ...
    def test_correct_user_registration(self):
        self.client().get("/users/register", follow_redirects=True)
    ...
    def test_incorrect_user_registration_with_different_passwords(self):
        self.client().get("/users/register", follow_redirects=True)
        response = self.register(
            "carlos", "montoya", "carlosmontoya89", "123456", "12345678"
        )
    ...
    def test_incorrect_user_registration_duplicate_username(self):
        ...
        self.assertEqual(response.status_code, 200)
        response = self.register(
            "charles", "montoya", "carlosmontoya89", "123456", "123456"
        )
    ...
    def test_correct_login(self):
        self.client().get("/users/register", follow_redirects=True)
        response = self.register(
            "carlos", "montoya", "cmontoya89", "validpassword", "validpassword"
        )
        self.assertEqual(response.status_code, 200)
        self.client().get("/users/login", follow_redirects=True)
